...
const ReviewArticleForm = () => {
  ...
  const handleGenerateReviewArticle = async () => {
    // Verificar que hay resúmenes seleccionados
    ...
    try {
      ...
      // Determinar si usar el prompt predefinido o las instrucciones personalizadas
      // Si useDefaultPrompt es true, se pasará una cadena vacía y el backend usará el prompt predefinido
      // Si useDefaultPrompt es false, se usarán las instrucciones específicas del usuario
      let finalInstructions = '';
      
      if (useDefaultPrompt) {
        // Si es el prompt predefinido, usar una cadena vacía o un placeholder
        finalInstructions = 'Usar formato predefinido UBoost'; // Esto es solo un placeholder, el backend usará el prompt predefinido
      } else {
        // Si es personalizado, verificar que hay instrucciones
        finalInstructions = specificInstructions.trim();
        if (finalInstructions === '' && !useDefaultPrompt) {
          // Si no hay instrucciones y no se usa el prompt predefinido, mostrar un error
          throw new Error('Por favor, ingrese instrucciones específicas o utilice el formato predefinido UBoost');
        }
      }
      
      // Incluir el flag para indicar si debe usar el prompt predefinido
      params.useDefaultPrompt = useDefaultPrompt;
      
      await generateReviewArticle(language, model, params, finalInstructions);
      ...
      <Paper sx={{ p: 3, borderRadius: 2, boxShadow: 2, mb: 3 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
            <FormControl component="fieldset">
              <Grid component="label" container alignItems="center" spacing={1}>
                <Grid item>
                  <Typography variant="body2" sx={{ fontWeight: useDefaultPrompt ? 400 : 500, color: useDefaultPrompt ? '#757575' : '#212121' }}>
                    Personalizado
                  </Typography>
                </Grid>
                <Grid item>
                ...
                <Grid item>
                  <Typography variant="body2" sx={{ fontWeight: useDefaultPrompt ? 500 : 400, color: useDefaultPrompt ? '#212121' : '#757575' }}>
                    Formato académico UBoost
                  </Typography>
                </Grid>
              </Grid>
            </FormControl>
            <Tooltip title="El formato académico UBoost genera un artículo de revisión completo con todas las secciones necesarias siguiendo los estándares científicos.">
        ...
        {useDefaultPrompt ? (
          <Alert severity="info" sx={{ mb: 3 }}>
            <Typography variant="subtitle2" gutterBottom>Se utilizará el prompt predefinido UBoost para artículos de revisión científica</Typography>
            <Typography variant="body2">El artículo incluirá: título, abstract, introducción, metodología, resultados, discusión, conclusiones y referencias siguiendo el formato académico estándar.</Typography>
          </Alert>
        ) : (
          <>
            <Alert severity="warning" sx={{ mb: 3 }}>
              <Typography variant="subtitle2" gutterBottom>Estás usando instrucciones personalizadas</Typography>
              <Typography variant="body2">Define claramente las secciones y el formato que deseas para tu artículo de revisión. Se recomienda especificar el formato académico deseado.</Typography>
            </Alert>
            ...
            <>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
                ...
                <Button
                  ...
                  sx={{
                    ...getPrimaryButtonStyles(selectedBatchSummaries.length === 0 || reviewArticleLoading),
