...
const ApiKeysConfig = ({ adminView = false }) => {
  ...
  const checkKeysStatus = async () => {
    try {
      const response = await apiClient.get('/api/config/api-keys-status');
  ...
  const validateOpenAIKey = async () => {
    ...
    try {
      const response = await apiClient.post('/api/config/validate-openai-key', { apiKey: openaiKey });
  ...
  const validateAnthropicKey = async () => {
    ...
    try {
      const response = await apiClient.post('/api/config/validate-anthropic-key', { apiKey: anthropicKey });
  ...
  const saveKeys = async () => {
    try {
      await apiClient.post('/api/config/save-api-keys', {
  ...
  // Renderizado condicional basado en si estamos en vista de (U)Boost o no
  return (
    <>
      {adminView ? (
        <Button 
