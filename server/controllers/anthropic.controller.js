...
exports.processQuery = async (req, res) => {
  try {
    ...
    if (req.user && req.user.id) {
      // Registrar el modelo correctamente
      await updateTokenUsage(req.user.id, tokenUsage, pdfFiles.length, model);
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        tokenUsage: tokenUsage,
        model: model  // Usar la variable model en lugar de un valor hardcodeado
      }
    });
