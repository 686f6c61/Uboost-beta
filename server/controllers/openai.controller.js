...
exports.generateReviewArticleSection = async (req, res) => {
  try {
    ...
    // Registrar el uso de tokens en MongoDB si hay un usuario autenticado
    if (req.user && req.user.id) {
      // No estamos procesando PDFs directamente en esta función, así que ponemos 0
      await updateTokenUsage(req.user.id, tokenUsage, 0);
    }
...
exports.generateSummary = async (req, res) => {
  try {
    ...
    // Verificar si se debe usar el prompt predefinido o personalizado
    const useDefaultPrompt = req.body.useDefaultPrompt !== undefined ? req.body.useDefaultPrompt : true;
    
    let fullPrompt = '';
    if (useDefaultPrompt) {
      // Usar el prompt predefinido desde prompts.js
      const summaryPrompt = getSummaryPrompt(query, language);
      const fileSpecificPrompt = getFileSpecificPrompt(fileIds);
      fullPrompt = summaryPrompt + fileSpecificPrompt;
    } else {
      // Usar un prompt personalizado
      fullPrompt = `
        Eres un asistente especializado en resumir documentos académicos y científicos. 
        Genera un resumen estructurado basado en los documentos proporcionados.
        Sigue las instrucciones específicas del usuario y mantén un estilo académico y profesional.
        Si la instrucción del usuario no es clara, usa tu mejor juicio para crear un resumen útil.
        Formato en markdown para mejorar la legibilidad.
      `;
    }
    ...
    if (useDefaultPrompt) {
      // Si usamos el prompt predefinido, usamos la estructura original
      messages = [
        {
          role: "system",
          content: fullPrompt
      ...
      // Si es un prompt personalizado, incluimos la instrucción del usuario en el mensaje principal
      messages = [
        {
          role: "system",
          content: fullPrompt
        ...
        {
          role: "user",
          content: `DOCUMENT CONTEXT:\n${context}\n\nINSTRUCCIONES DEL USUARIO PARA EL RESUMEN:\n${query}`
        }
      ];
    }
    ...
    // Registrar el uso de tokens en MongoDB si hay un usuario autenticado
    if (req.user && req.user.id) {
      // Determinar el número de PDFs procesados
      const pdfsCount = fileIds ? fileIds.length : pdfFiles.length;
      
      // Actualizar el uso de tokens para el usuario
      await updateTokenUsage(req.user.id, tokenUsage, pdfsCount);
    }
